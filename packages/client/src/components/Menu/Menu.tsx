...
export class Menu extends React.Component<MenuProps, {}> {
    ...
    private getPosition = (position: string, originElement: HTMLElement) => {
        ...
        const menuHeight = this.props.buttonHeight * this.props.menuItems.length * fontSize;
        ...
        switch (position) {
            case 'right': {
                menuStyle.right = getBoundingPos(coords.right, menuWidth, window.innerWidth);
                menuStyle.bottom = getBoundingPos(coords.top, menuHeight, window.innerHeight);
                break;
            }
            default: {
                menuStyle.right = '0';

...
class TrecipeService {
    ...
    public fetchAllTrecipes(): Promise<Array<Trecipe>> {
        return API.get<Array<Trecipe>>(`${baseURL}/${this.apiEndpoint}`).then(
    ...
    public createTrecipe(trecipeData: CreateNewTrecipeDTO): Promise<Trecipe> {
        return API.post<Trecipe>(`${baseURL}/${this.apiEndpoint}`, trecipeData).then(
    ...
    public duplicateTrecipe(srcTrecipeId: string): Promise<Trecipe> {
        return API.post<Trecipe>(`${baseURL}/${this.apiEndpoint}/copy`, null, {
            params: {
                id: srcTrecipeId,
            },
    ...
    public deleteTrecipe(idToDelete: string): Promise<number> {
        return API.delete(`${baseURL}/${this.apiEndpoint}/${idToDelete}`).then((res) => {
    ...
    public updateTrecipe(idToUpdate: string, updatedTrecipe: Partial<Trecipe>): Promise<Trecipe> {
        return API.put<Trecipe>(`${baseURL}/${this.apiEndpoint}/${idToUpdate}`, updatedTrecipe).then(
            (res: AxiosResponse<Trecipe>) => {
                return Promise.resolve(res.data);
            }
        );
    ...
    public getTrecipe(id: string): Promise<Trecipe> {
        return API.get<Trecipe>(`${baseURL}/${this.apiEndpoint}/${id}`).then((res: AxiosResponse<Trecipe>) => {
            return Promise.resolve(res.data);
        });
    }
}
